...
The delta method approximates teh variance of each $SAR$ variant as,
...
Four variants of progeny-per-parent ($P/P$) ratios are calculated to estimate trends in adult-to-adult productivity. The four variants include: (1) total recruits per total spawner, (2) adult recruits per adult spawner, (3) total recruits per adult spawner, and (4) female recruits per female spawner. Estimation of each $P/P$ ratio follows a similar format using total recruits ($\hat{N}_{BY}$), adult recruits ($\hat{N}_{BY,EJ}$), and female recruits ($\hat{N}_{BY,F}$) from section \@ref(sec:agereturn) in the numerator, and total spawners ($\hat{S}_{T}$), adult spawners ($\hat{S}_{T,EJ}$) and female spawners ($\hat{S}_{T,F}$; Section \@ref(sec:spawnerabundance)) in the denominator.
...
The variance approximation for the four variations of progeny-per-parent is derived from the delta method as

\begin{equation}
Var(\hat{P/P}_1) = \bigg(\frac{1}{\hat{S}^2_{T}}\bigg) \; Var(\hat{N}_{BY}) + \bigg(\frac{\hat{N}_{BY}^2}{\hat{S}^4_{T}}\bigg) \; Var(\hat{S}_{T}).
\end{equation}

## Recruits-per-Spawner {#sec:rpers}

### Spawners Above Juvenile Trap {#sec:spawnerstrap}
Calculating juvenile recruits-per-spawner requires an estimation of fish spawning above a juvenile emigrant trap. The inclusion of all tributary spawners in recruit per spawner values biases the productivity estimate low. To avoid the bias, the spatial location of tributary spawners is assumed equal to redd location. As such, the proportion of redds above a juvenile trap ($(R_J/R_T)$; Section \@ref(sec:reddcount)) is applied to tributary spawners ($\hat{S}_T$; \@ref(sec:spawner)) to estimate spawners above emigrant traps ($\hat{S}_{J}$).
...
Again, assuming redds are known without error the variance becomes
...
Six variants of juvenile recruit per spawner values are calculated to show productivity trends; they include: (1) emigrant per total spawner, (2) emigrant per adult spawner, (3) emigrant per female spawner, (4) smolt per total spawner, (5) smolt per adult spawner, and (6) smolt per female spawner.
...
Where the numerator $\hat{N}_{J}$ is the total emigrant abundance at the trap (Section \@ref(sec:juvenileabundance)) or $\hat{N}_{S}$, the estimated number of smolts at Lower Granite Dam (\@ref(sec:smolts)). And the denominator consists of all spawners ($\hat{S}_{J}$), spawners excluding jacks ($\hat{S}_{J,EJ}$) or female spawners ($\hat{S}_{J,F}$) above the juvenile trap. Where the three types of spawners above the trap are calculated following methods outlined in section \@ref(sec:spawnerstrap). The variance for the six variations of recruits-per-spawner is approximated similarly with the delta method as follows;
